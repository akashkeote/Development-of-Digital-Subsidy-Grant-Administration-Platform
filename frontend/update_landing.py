...
# 1. Inject missing imports for the canvas logic
...
# 2. Inject the hooks and state into the LandingPage component
hooks_code = """
...
# Insert hooks right after "export const LandingPage: React.FC = () => {"
if "const canvasRef" not in code:
    code = code.replace("export const LandingPage: React.FC = () => {\n", "export const LandingPage: React.FC = () => {\n" + hooks_code)
    code = code.replace("export function LandingPage() {\n", "export function LandingPage() {\n" + hooks_code)

# 3. Replace the Hero Section with the Canvas Section
hero_start = code.find("{/* Hero Section */}")
visionary_start = code.find("{/* Visionary Leaders Graphic Section */}")
...
code = code[:hero_start] + scrolly_hero + code[visionary_start:]

# Change the outermost container class to remove bg-slate-50 which might clash
code = code.replace('<div className="min-h-screen bg-slate-50 relative overflow-hidden flex flex-col">', '<div className="min-h-screen relative overflow-hidden flex flex-col bg-[#050505]">')
...
with open(r"D:\Government-Subsidy-Grant-Disbursement-Tracking-System\frontend\src\pages\LandingPage.tsx", "w", encoding="utf-8") as f:
    f.write(code)
print("Updated LandingPage.tsx with Scrollytelling Canvas Hero!")
